...
public class UpdateVisitPlanListPresenter {
    private GlobalPresenter globalPresenter;

    public UpdateVisitPlanListPresenter (GlobalPresenter globalPresenter){
        this.globalPresenter = globalPresenter;
    }

    public void updateVisitPlanList (String token, int id, int visit_plan_id, int customer_id, int day, int type, String description){
        ...
        ApiConnection.getInstance().getRoutes().updateVisitPlanList("Bearer ".concat(token),id, data).enqueue(new Callback<Message>() {
            @Override
