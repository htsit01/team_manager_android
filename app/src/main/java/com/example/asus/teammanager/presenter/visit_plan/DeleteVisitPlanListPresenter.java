...
public class DeleteVisitPlanListPresenter {
    private GlobalPresenter globalPresenter;

    public DeleteVisitPlanListPresenter (GlobalPresenter globalPresenter){
        this.globalPresenter = globalPresenter;
    }

    public void deleteVisitPlanList(String token, int plan_id , int visit_plan_id){
        Map<String, Integer> data = new HashMap<>();
        data.put("visit_plan_id", visit_plan_id);

        ApiConnection.getInstance().getRoutes().deleteVisitPlanList("Bearer ".concat(token), plan_id, data).enqueue(new Callback<Message>() {
            @Override
