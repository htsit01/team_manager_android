...
public class FollowUpCheckinPresenter {
    private GlobalPresenter globalPresenter;

    public FollowUpCheckinPresenter(GlobalPresenter globalPresenter){
        this.globalPresenter = globalPresenter;
    }

    public void doCheckin(String token, int followup_id, double lat, double lng, String address, String start_time, int type){
        ...
        ApiConnection.getInstance().getRoutes().doFollowUpCheckin("Bearer ".concat(token),data).enqueue(new Callback<Message>() {
            @Override
